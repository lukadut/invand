...
import aplmob.dutkastudencki.spaceinvader.game.objects.ChickenLeg;
...
public class Renderer {
    ...
    public void render(float delta){
        ...
        // petla po wszystki nogach kurczaka
        Iterator<GameObject> legsIterator = world.getLegs().iterator();
        while(legsIterator.hasNext()) {
            GameObject leg = legsIterator.next();
            leg.move();
            leg.draw(batch);
            if (leg.getBoundingBox().overlaps(ship.getBoundingBox())) {
                world.killedLeg();
                legsIterator.remove();
                leg.dispose();
                continue;
